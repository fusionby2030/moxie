...
def main(): 
    ...
    train_dl, val_dl = get_train_test_splits() 
    ...
    for EPOCH in range(EPOCHS): 
        for n, batch in enumerate(train_dl): 
            ...
            print(f'{EPOCH}/{n}: Loss {loss.item()}, Recon t0: {recon_loss.item()}, Unsup: {kld_loss.item()}, Phys: {physics_loss.item()}')
    ...
    save_dict = {'state_dict': model.state_dict(), 
                'train_set': train_set, 
                'val_set': val_set}
    torch.save(save_dict, '/home/kitadam/ENR_Sven/test_moxie/experiments/SRL_meditations/model_results/STEP_2_0.pth')
