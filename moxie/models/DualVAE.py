...
class DualVAE(BaseVAE):
    ...
    You have a Encoder -> (z_stochastic, z_machine) -> Decoder
    ...
    def encode(self, input: Tensor) -> List[Tensor]:
        ...
        mu_stoch = self.mu_stoch(result)
        log_var_stoch = self.mu_stoch(result)

        mu_mach = self.mu_mach(result)
        log_var_mach = self.mu_mach(result)
